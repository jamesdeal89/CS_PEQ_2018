...
# Q11
def GetScoreForWordAndPrefix(Word,AllowedWords,TileDictionary,Total,End):
  if len(Word) <= 1:
    return Total
  elif CheckWordIsValid(Word,AllowedWords,0,len(AllowedWords)-1):
    Total += GetScoreForWord(Word,TileDictionary)
    print(Total)
  End -= 1
  return GetScoreForWordAndPrefix(Word[:End],AllowedWords,TileDictionary,Total,End)
...
def CheckWordIsValid(Word, AllowedWords, start, end):
  """
  Count = 0
  ...
  if start <= end:
    ...
    if AllowedWords[mid] == Word:
      ...
      return CheckWordIsValid(Word, AllowedWords, mid+1, end)
    elif AllowedWords[mid] > Word:
      return CheckWordIsValid(Word, AllowedWords, start, mid-1)
  else:
    return False
...
def UpdateAfterAllowedWord(Word, PlayerTiles, PlayerScore, PlayerTilesPlayed, TileDictionary, AllowedWords):
  ...
  PlayerScore += GetScoreForWordAndPrefix(Word, AllowedWords, TileDictionary, 0, len(Word))
...
def HaveTurn(PlayerName, PlayerTiles, PlayerTilesPlayed, PlayerScore, TileDictionary, TileQueue, AllowedWords, MaxHandSize, NoOfEndOfTurnTiles):
  ...
  while not ValidChoice:
    ...
    if Choice == "1":
      ...
      if ValidWord:
        ValidWord = CheckWordIsValid(Choice, AllowedWords, 0, len(AllowedWords)-1)
        if ValidWord:
          print()
          print("Valid word")
          print()
